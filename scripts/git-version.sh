#!/bin/sh
# The single definition of the version of the package this repository
...
# version are a reachable tag or an explicit CONSOLE_VERSION.
#
...
emit_checked() {
    if ! printf '%s' "$1" | grep -Eq '^[0-9][A-Za-z0-9.+~]*$'; then
        ...
        echo "  Rename the tag (v1.2.3 or v1.2.3-alpha1) or pass CONSOLE_VERSION." >&2
        exit 1
    fi
...
if [ -n "${CONSOLE_VERSION:-}" ]; then
    emit_checked "$CONSOLE_VERSION" "CONSOLE_VERSION"
    exit 0
fi

if ! git rev-parse --git-dir >/dev/null 2>&1; then
    echo "git-version.sh: no .git directory and CONSOLE_VERSION is unset." >&2
    echo "  Building outside a git checkout? Pass the version explicitly:" >&2
    echo "    make <target> CONSOLE_VERSION=0.1.0" >&2
    exit 1
fi
...
if ! describe=$(git describe --tags --match 'v[0-9]*' --dirty=+dirty 2>/dev/null); then
    ...
    echo "  (git fetch --tags), or pass CONSOLE_VERSION=<version> explicitly." >&2
    exit 1
fi
