...
// `client/web/src/elements/`, served by the daemon at `/elements/vstimd.js`.
// It lives here only so experiment 02 can run before that change is made; see
// ../README.md.
//
// statemachined already publishes this contract (dev/DAEMON.md §5) and names
// `braemons-console` as its consumer. vstimd cannot join a console until it
// publishes the same thing, because the console holds no domain logic: every
// panel is served by the daemon that owns the hardware it is about.
//
// The awkward part is that vstimd's UI is React and the contract is custom
...
import { Connection, type SceneSnapshot } from "@vstimd/web/src/index.js";
import { StimuliPanel } from "@vstimd/web/src/app/StimuliPanel.js";
import { StimulusMap } from "@vstimd/web/src/app/StimulusMap.js";
import { VtlPanel } from "@vstimd/web/src/app/VtlPanel.js";
import { AnimationsPanel } from "@vstimd/web/src/app/AnimationsPanel.js";
import { SystemPanel } from "@vstimd/web/src/app/SystemPanel.js";
 ...
 *
 * "connecting" and "unreachable" have to be different words on the screen, and
 * finding that out is what experiment 02 was for: with vstimd stopped, a panel
 * that only knows connected/not sat on "connecting…" indefinitely, which reads
 * as *slow* when it means *absent*. A person deciding whether to walk to the
 ...
 * from the rig -- the same reason statemachined's elements take one. An http(s)
 * base is accepted and converted, since that is what an mDNS record and a
 * person both produce.
 */
